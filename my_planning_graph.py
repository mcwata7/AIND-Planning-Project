...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            A = PgNode_a(action)
            if all(value in self.s_levels[level] for value in A.prenodes):
                for literal in self.s_levels[level]:
                    if literal in A.prenodes:
                        A.parents.add(literal)
                level_actions.add(A)

        self.a_levels.append(level_actions)
    ...
    def add_literal_level(self, level):
        ...
        self.s_levels.append(level_literals)    
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            ...
            found = False
            while not found:
                if PgNode_s(goal, True) in self.s_levels[i]:
                    found = True
                else:
                    i += 1
